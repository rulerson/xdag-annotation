...
{
    static int count = 0;
    if (!(mode & DFSRSA_DEBUG_MODE))
        return -1;
    if (count >= DFSRSA_DEBUG_PRINT_MAX)
        return -1;
    count++;
    printf("%16s = 0x", name);
    while (len--) {
...
#if !defined(USE_KARATSUBA) || KARATSUBA_TRESHOLD > 64
    if (len == 64) {
        dfsrsa_fastmul64_x86_64(prod, mul1, mul2);
        return;
    }
#endif
#if !defined(USE_KARATSUBA) || KARATSUBA_TRESHOLD > 32
    if (len == 32) {
        dfsrsa_fastmul32_x86_64(prod, mul1, mul2);
        return;
    }
#endif
#if !defined(USE_KARATSUBA) || KARATSUBA_TRESHOLD > 16
    if (len == 16) {
        dfsrsa_fastmul16_x86_64(prod, mul1, mul2);
        return;
    }
#endif
#endif
...
#if !defined(USE_KARATSUBA) || KARATSUBA_TRESHOLD > 64
    if (len == 64) {
        dfsrsa_fastmul64_arm(prod, mul1, mul2);
        return;
    }
#endif
#if !defined(USE_KARATSUBA) || KARATSUBA_TRESHOLD > 32
    if (len == 32) {
        dfsrsa_fastmul32_arm(prod, mul1, mul2);
        return;
    }
#endif
#if !defined(USE_KARATSUBA) || KARATSUBA_TRESHOLD > 16
    if (len == 16) {
        dfsrsa_fastmul16_arm(prod, mul1, mul2);
        return;
    }
#endif
#endif
...
{
#ifdef DFSRSA_DEBUG
    dfsrsa_t *m1 = mul1, *m2 = mul2, *p = prod, *w = work;
    int l = len;
#endif
    int r, llen;
    dfsrsa_long_t rem;
    ...
    if (r > 0) {
        while (llen--) {
            rem += (dfsrsa_long_t)*prod + (dfsrsa_long_t)*mul1++ + (dfsrsa_long_t)*mul2++ - (dfsrsa_long_t)*work++;
            *prod++ = (dfsrsa_t)rem;
...
{
    int res, offset;
    dfsrsa_long_t mod0, div0, div1, div2;
    dfsrsa_debug(MOD, "divisor", mod, mlen);
    dfsrsa_debug(MOD, "dividend", div, len);
    len = dfsrsa_getlen(div, len);
    if (!len)
                          ...
                          dfsrsa_t *work, int len)
{
    ...
    while (!(*mod4 & 1))
        dfsrsa_shr1(mod4, len, 0), mask <<= 1, mask |= 1;
    ...
    for (;;) {
        ...
        while (!(*pn & 1)) {
            ...
            if (*px & 1)
                carry = dfsrsa_add(px, px, mod4, len);
        ...
        while (!(*qn & 1)) {
            ...
            if (*qx & 1)
                carry = dfsrsa_add(qx, qx, mod4, len);
...
fin:
    dfsrsa_set(qn, len, 1);
    if (dfsrsa_cmp(pn, qn, len))
        return -1;
    dfsrsa_debug(INV, "inv mod/4", inv, len);
    while ((*num ^ *inv) & mask)
        dfsrsa_add(inv, inv, mod4, len);
                                  ...
                                  int len)
{
    ...
    if (dfsrsa_inverse(mdata + 3 * len, mdata, mod, work, len))
        return -1;
    dfsrsa_debug(MONT, "montgomery R^-1", mdata + 3 * len, len);
    if (dfsrsa_divmod(mdata + 2 * len, 2 * len, mod, len, mdata))
        return -1;
                                    ...
                                    int len)
{
    ...
    if (dfsrsa_mod(mdata + 2 * len, 2 * len, mod, len))
        return -1;
...
#define kwork (mdata + 4 * len)
    ...
    if (dfsrsa_getlen(num, len)) {
        ...
        if (dfsrsa_adc(num, num + len, mdata + 3 * len, 1, len) || dfsrsa_cmp(num, mod, len) >= 0) {
...
#define kwork (work + 8 * len)
    ...
    int flag = 0;
    int i;
    if (dfsrsa_montgomery_init(mod, mdata, work, len))
        goto powmod;
    dfsrsa_set(work, len, 1);
    if (dfsrsa_montgomery_reduce(work, mod, mdata, len))
        goto powmod;
    dfsrsa_copy(mbase, base, len);
    if (dfsrsa_montgomery_reduce(mbase, mod, mdata, len))
        goto powmod;
    int explen = dfsrsa_getlen(exp, len);
    while (explen--) {
        bits = exp[explen], i = DFSRSA_T_BITS;
    ...
    return;
powmod:
    perror("Montgomery_powmod failed! Using std powmod...");
    ...
    for (d = 2; d < lim; ++d, d |= 1)
        ...
        if (dfsrsa_isprime0(d)) {
            *a = d;
            ...
            if ((dfsrsa_cmp(work, o, len) && dfsrsa_cmp(work, nm1, len)) || (dfsrsa_cmp(work2, o, len) && dfsrsa_cmp(work2, nm1, len)))
                return 0;
        }
    return 1;
}
...
int dfsrsa_crypt(dfsrsa_t *data, int datalen, dfsrsa_t *key, int keylen)
{
    ...
    if (keylen <= 0 || (keylen & 1) || datalen < 0 || datalen % len)
        return -1;
    datalen /= len;
    for (i = 0; i < datalen; ++i) {
        if (dfsrsa_cmp(data + i * len, mod, len) >= 0)
            return -1;
    }
...
int work(int keylen, int datalen)
{
    ...
    if (dfsrsa_keygen(privkey, pubkey, keylen))
        return -3;
    ...
    if (dfsrsa_crypt(data, datalen, pubkey, keylen))
        return -4;
    dfsrsa_debug(TOP, "data+pub", data, datalen);
    if (!dfsrsa_cmp(data, data0, datalen))
        return -5;
    if (dfsrsa_crypt(data, datalen, privkey, keylen))
        return -6;
