...
int dfslib_encrypt_sector(struct dfslib_crypt *dfsc, dfs32 *sector, dfs64 sectorNo)
{
    ...
    while (i--)
        dfs_encrypt6(a, b, c, d, x, y, z, t, sector);
    return 1;
}
...
{
    dfs32 a, b, c, d, x, y, z, t;
    if (!dfsc->ispwd || (size & 1))
        return 0;
    dfs_prepare(dfsc, sectorNo, &x, &y, &z, &t);
    dfs_enmixArray(data, size);
    size >>= 1;
    while (size--)
        dfs_encrypt3(a, b, c, d, x, y, z, t, data);
    return 1;
}
...
int dfslib_uncrypt_sector(struct dfslib_crypt *dfsc, dfs32 *sector, dfs64 sectorNo)
{
    ...
    dfs_prepare(dfsc, sectorNo, &x, &y, &z, &t);
    while (i--)
        dfs_uncrypt6(a, b, c, d, x, y, z, t, sector);
                         ...
                         dfs64 sectorNo)
{
    ...
    size >>= 1;
    while (size--)
        dfs_uncrypt3(a, b, c, d, x, y, z, t, data);
