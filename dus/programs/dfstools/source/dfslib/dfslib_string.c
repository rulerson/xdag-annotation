...
int dfslib_unicode_cmp(const struct dfslib_string *str, unsigned *ptr, int unicode)
{
    ...
    if (res < 0 && res != DFSLIB_NAME_TOO_LONG)
        return res;
...
{
    if (str->type == DFSLIB_STRING_UNICODE)
        return str->len;
    else {
        unsigned ptr = 0, count = 0;
        int res;
        while ((res = dfslib_unicode_read(str, &ptr)) >= 0)
            ++count;
                          ...
                          unsigned *ptr)
{
    unsigned begin, end;
    int res;
    while (begin = *ptr, (res = dfslib_unicode_read(str, ptr)) >= 0 && dfslib_unicode_strchr(limits, res) >= 0)
        ;
    if (res < 0)
        return res;
    while (end = *ptr, (res = dfslib_unicode_read(str, ptr)) >= 0 && dfslib_unicode_strchr(limits, res) < 0)
        ;
    if (res < 0 && res != DFSLIB_NAME_TOO_LONG)
        return res;
