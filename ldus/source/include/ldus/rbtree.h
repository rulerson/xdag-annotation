...
static inline void ldus_rbtree_walk_up(struct ldus_rbtree *root, void (*callback)(struct ldus_rbtree *node))
{
    if (root) {
        ...
        if (root->left)
            ldus_rbtree_walk_up(root->left, callback);
        if (root->right)
            ldus_rbtree_walk_up(root->right, callback);
        callback(root);
    }
}
...
static inline void ldus_rbtree_walk_right(struct ldus_rbtree *root, void (*callback)(struct ldus_rbtree *node))
{
    while (root) {
        root = _rbtree_ptr(root);
        if (root->left)
            ldus_rbtree_walk_right(root->left, callback);
        callback(root);
        root = root->right;
    }
}
...
#define ldus_rbtree_define_prefix(lessthan, prefix, postfix)                                                \
    ...
    {                                                                                                       \
        ...
        if (!root) {                                                                                        \
            *proot = node, node->left = node->right = 0;                                                    \
            return 1;                                                                                       \
        }                                                                                                   \
        if (lessthan(node, root))                                                                           \
            return _rbtree_insert_balance_left(proot, ldus_rbtree_insert(&root->left, node));               \
        if (lessthan(root, node))                                                                           \
            return _rbtree_insert_balance_right(proot, ldus_rbtree_insert(&root->right, node));             \
        return -1;                                                                                          \
    }                                                                                                       \
                                                                                                            \
    ...
    {                                                                                                       \
        ...
        if (!root)                                                                                          \
            return 0;                                                                                       \
        if (lessthan(node, root))                                                                           \
            return ldus_rbtree_find(root->left, node);                                                      \
        if (lessthan(root, node))                                                                           \
            return ldus_rbtree_find(root->right, node);                                                     \
        return root;                                                                                        \
    }                                                                                                       \
                                                                                                            \
    ...
    {                                                                                                       \
        ...
        if (!root)                                                                                          \
            return -1;                                                                                      \
        if (lessthan(node, root))                                                                           \
            return _rbtree_remove_balance_left(proot, ldus_rbtree_remove(&root->left, node));               \
        if (lessthan(root, node))                                                                           \
            return _rbtree_remove_balance_right(proot, ldus_rbtree_remove(&root->right, node));             \
        if (root->left)                                                                                     \
            return _rbtree_remove_balance_left(proot, _rbtree_remove_left(&root->left, proot));             \
        if (root->right)                                                                                    \
            return _rbtree_remove_balance_right(proot, _rbtree_remove_right(&root->right, proot));          \
        if (_rbtree_color(*proot)) {                                                                        \
            *proot = 0;                                                                                     \
            return 0;                                                                                       \
        }                                                                                                   \
