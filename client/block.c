...
/*
 ...
 amount: 地址余额，包含所有转入这个地址的金额总和
 linkamout: 对应link块本次的in/out金额
 
 
 in_mask: 表示哪个是in field
 ...
 fee：手续费
 time: 时间戳
 storage_pos: block在时间片文件中的位置偏移。
 link: in/out 块引用
 nlinks：实际link数量。
 max_diff_link：第几个link是最大难度路径。
 ref: link自己的block。注意是主链路径上过来的那个引用，只有一个，与下面backrefs有所区分。
 backrefs：包含所有反向引用自己的block指针。
 remark：还没看懂什么意思
 flags：参见bi_flags
 in_mask：标记所有link中，哪些是infield，其他的就是outfield
 n_our_key: key编号，属于自己的块才有，说明这个block是自己的key链上第几个key做的签名。
...
/*
 增加amount
 如果是BI_OURS的block
    调整ourfirst链， 把bi按照amount排序后放到正确的位置, 大的在ourfirst方向
 */
...
/*
 递归遍历当前block开始链接起来的tree，调整所有block的金额字段，使得每个block的金额都能反应以block为地址时的余额，完成后标记为BI_APPLIED|BI_MAIN_REF
 
 sum_in：别的block输入给这个block的金额总和
 sum_out：包含两部分
    1. 当前block输出给其他block的金额
    2. 当前block交易完成后付出的手续费，目前为0。
 
 fee是这笔交易的手续费，所以金额必须从input里面出，不是从当前block的余额出。
 当前block的余额在执行完之前是不存在的，因为当前block还没被网络接受。只有当前block被网络接受后，作为当前block的地址，余额才能算数。
 */
static uint64_t apply_block(struct block_internal *bi)
{
    xdag_amount_t sum_in, sum_out;  // input/output总金额
    ...
    for (int i = 0; i < bi->nlinks; ++i) {
        if (1 << i & bi->in_mask) {
            if (bi->link[i]->amount < bi->linkamount[i]) {  // link block的转出金额不能比自己的余额多
                return 0;
            }
            if (sum_in + bi->linkamount[i] < sum_in) {      // 不可溢出，变小表示溢出了。
                return 0;
            }
            sum_in += bi->linkamount[i];
        } else {
            if (sum_out + bi->linkamount[i] < sum_out) {    // 不可溢出
                return 0;
            }
    ...
    // sum_in + bi->amount < sum_in : false  ?
    if (sum_in + bi->amount < sum_in || sum_in + bi->amount < sum_out) {   // 不可溢出
        return 0;
    }

    // 对每个link块，计算并增减对应的in/out金额
    for (int i = 0; i < bi->nlinks; ++i) {
        if (1 << i & bi->in_mask) {
...
/*
 block退出主块、回退关联block。
 回退block：从当前block开始，直到与有效主链不重合的所有block。
 */
...
/*
 设置block为BI_MAIN（main block）
 
 为block增加挖矿金额
 把内存中以block为起点的链接树上所有block的金额都调整为以各自block为地址时候的余额
 block自己的fee加到自己的amount上
 完成后block增加标记：BI_MAIN|BI_APPLIED|BI_MAIN_REF
 */
static void set_main(struct block_internal *m)
{
    ...
    accept_amount(m, apply_block(m)); // 当前交易手续费加到自己的amount
    m->ref = m;
...
/*
 设置一个新的main block
    从topmainchain开始，顺着maxdifflink找下去
    找到最后一个设置了BI_MAIN_CHAIN但没有设置BI_MAIN的block
    如果这个block的time比当前早了2秒以上，且总共找到的这种类型的block量超过MAX_WAITING_MAIN个(1)
 完成后block必须有的标记：BI_MAIN_CHAIN|BI_MAIN|BI_APPLIED|BI_MAIN_REF
 */
...
// 从top_main_chain到b之前的主链上的块，都取消BI_MAIN_CHAIN标记，如果有BI_MAIN标记，取消BI_MAIN
...
// hash(block#1-byte-flag#pubkeydata), 1-byte-flag: binay-11 or 10
...
// 验证signo_r开始的两个field的sign，并返回验证成功的pubkey index
static int valid_signature(const struct xdag_block *b, int signo_r, int keysLength, struct xdag_public_key *keys)
{
    ...
    for (i = signo_r; i < XDAG_BLOCK_FIELDS; ++i) {
        if (xdag_type(b, i) == XDAG_FIELD_SIGN_IN || xdag_type(b, i) == XDAG_FIELD_SIGN_OUT) {
            memset(&buf[0].field[i], 0, sizeof(struct xdag_field)); // 清空所有sign字段
            if (i > signo_r && signo_s < 0 && xdag_type(b, i) == xdag_type(b, signo_r)) {
                signo_s = i; // 第一个与signo_r field同类型的field
            }
        }
    }
...
/* 
 b早于当前时间片
 如果pre_top没有，直接设置。如果有，比较diff，大的设置为pretop_main_chain
 */
#define set_pretop(b)                                                                           \
 ...
 *		<0 = error
 拦截错误block
 验证in/out字段的签名、并且验证关联块的out签名、如果输出有自己的签名，则标记为BI_OURS
 存储块到内存与磁盘
 计算diff值，包含下一个ref的值。diff若是最大，则设置为top_main_chain。
 计算全局hashrate
 
 err: 低4位为error code，高四位为时间片序号的低4位。
...
static int add_block_nolock(struct xdag_block *newBlock, xtime_t limit)
{
    ...
    int signInCount = 0, signOutCount = 0;      // 记录有多少个签名field
    int signinmask = 0, signoutmask = 0;        // 与signInCount和signOutCount不同，只在前半个签名field位置做mask 1。
    ...
    /* 错误block条件
        1. block的时间戳比当前快1/4时间片长度
        2. 早于XDAG_ERA
        3. limit非0，比当前时间片小limit以上。目前limit固定为0。
     
     总结:
        若limit = 0，可接受的时间戳范围 XDAG_ERA <= t <= timestamp + MAIN_CHAIN_PERIOD / 4
        若limit > 0，可接受的时间戳范围 timestamp - limit <= t <= timestamp + MAIN_CHAIN_PERIOD / 4
     */
    if (tmpNodeBlock.time > timestamp + MAIN_CHAIN_PERIOD / 4 || tmpNodeBlock.time < XDAG_ERA || (limit && timestamp - tmpNodeBlock.time > limit)) {
        i = 0;
        err = 2;
        goto end;
    }
    ...
    /*
     识别出不同字段的类型
        - inmask/outmask，标记哪些field是input/output
        - signinmask/signoutmask, 标记哪些字段是signin/signout，注意，只标记第一个遇到的field编号，因为必须连续出现两个。
        - signInCount/signOutCount, 标记有多少个signin/signout字段。
     读取所有的pubkey 到 public_keys, 转化为内部结构体以便后续使用。
     */
    for (i = 1; i < XDAG_BLOCK_FIELDS; ++i) {
        switch ((type = xdag_type(newBlock, i))) {
            ...
            case XDAG_FIELD_OUT:
                outmask |= 1 << i;
                break;
                // sign包含连续的两个field，所以这里跳过一个字段，out也一样
            ...
            case XDAG_FIELD_SIGN_OUT:
                ...
                break;
            case XDAG_FIELD_PUBLIC_KEY_0: // 找出key field，把所有key字段复制出来。
    ...
    if (signOutCount & 1) {     // signout个数必须是偶数，当前只能是2。
        i = signOutCount;
        err = 4;
        goto end;
    }
    ...
    /* TODO: if not read from storage and timestamp is ...ffff and last field is nonce then the block is extra */
    if (!g_light_mode && (transportHeader & (sizeof(struct xdag_block) - 1)) && (tmpNodeBlock.time & (MAIN_CHAIN_PERIOD - 1)) == (MAIN_CHAIN_PERIOD - 1) && (signinmask & 1 << (XDAG_BLOCK_FIELDS - 1))) {
    ...
    for (i = 1; i < XDAG_BLOCK_FIELDS; ++i) {
        if (1 << i & (inmask | outmask)) {
            ...
            if (blockRefs[i - 1]->time >= tmpNodeBlock.time) {      // 引用了比自己更快的block
                err = 6;
                goto end;
            }
            if (tmpNodeBlock.nlinks >= MAX_LINKS) {     // 这里貌似不可能执行到，nlinks还没赋值
                err = 7;
                goto end;
            }
        }
    ...
    // 根据当前内存中的数据（不包含当前block），重新设置一次主链。
    if (!g_light_mode) {
        check_new_main();
    }
    ...
    /*
     用所有block中读取到的pubkey，对所有signin/signout字段都做一次验证签名操作，把能验证通过的pubkey记录下来。
     有任何一个signout字段能被自己的pubkey验证通过，标记block为BI_OURS。说明，这个block归属自己。
     把没有验证成功的pubkey的内存结构释放掉。意味着有部分block 中的 pubkey虽然传过来了，但有可能验证不通过。
     */
    for (i = 1; i < XDAG_BLOCK_FIELDS; ++i) {
    ...
    for (i = 1; i < XDAG_BLOCK_FIELDS; ++i) {
        if (1 << i & (inmask | outmask)) {
            ...
            // 对于一个in field，找到对应ref block中的signout field，验证对应ref block中signout签名, 主要是验证这个pubkey是否属于对应上游link block所有
            if (1 << i & inmask) {
            ...
            *psum += newBlock->field[i].amount;
            ...
            /* 
             计算diff，maxdiff link
                diff = max of all link_diff
                maxdiff link = maxdiff方向的link
             link_diff定义
                新定义变量
                    tmpNodeBlock_diff, 当前块diff，也就是diff0
                    link_block_diff, 搜索停止后指向的block diff
                    original_link_block_diff, 在link在同一个时间片，原始的link指向的block diff
                如果link在更早的时间片，link_diff=tmpNodeBlock_diff + link_block_diff
                如果link在相同时间片，沿着link block的maxdiff递归下一个block，遇到不同时间片或者为空停止
                    如果最后一个maxdiff block还在相同时间片      // 最后ref为空
                        link_diff = original_link_block_diff
                    如果最后maxdiff block在不同时间片           // 最后ref跨越了时间片
                        link_diff = max(link_block_diff + tmpNodeBlock_diff, original_link_block_diff）

             */
            if (MAIN_TIME(blockRef->time) < MAIN_TIME(tmpNodeBlock.time)) {
                diff = xdag_diff_add(diff0, blockRef->difficulty); // diff0为tmpNodeBlock的diff
            } else {
    ...
    if (g_xdag_extstats.nextra > MAX_ALLOWED_EXTRA && (g_xdag_state == XDAG_STATE_SYNC || g_xdag_state == XDAG_STATE_STST)) {
    ...
    if (!(transportHeader & (sizeof(struct xdag_block) - 1))) {     // storage_pos是以512位单位，所以如果是storage_pos，低位是0.
    ...
    // 如果nodeBlock, pretop_main_chain, top_main_chain三个的时间片都早于当前，三者中最大diff的设置为新的pretop_main_chain
    set_pretop(nodeBlock);
    set_pretop(top_main_chain);

    /* 
     tmpNodeBlock的diff若是最大，则设置为新的top_main_chain，顺着tmpNodeBlock开始标记新的主链，主链上的block为主块，主块引用的块都是有效的
     对老的top_main_chain到新的主链之间的block，全部反向操作，退出主链
     */
    if (xdag_diff_gt(tmpNodeBlock.difficulty, g_xdag_stats.difficulty)) {
        ...
        /*
         从当前block开始，顺着max diff link迭代标注为BI_MAIN_CHAIN, 直到迭代到头了，或者遇到了BI_MAIN_CHAIN的block
         几个条件
            1. 当前block比下个max diff block的diff值大，下个block为空也算比较成功
            2. 时间片必须按照由大到小
         */
        for (blockRef = nodeBlock, blockRef0 = 0; blockRef && !(blockRef->flags & BI_MAIN_CHAIN); blockRef = blockRef->link[blockRef->max_diff_link]) {
            if ((!blockRef->link[blockRef->max_diff_link] || xdag_diff_gt(blockRef->difficulty, blockRef->link[blockRef->max_diff_link]->difficulty)) && (!blockRef0 || MAIN_TIME(blockRef0->time) > MAIN_TIME(blockRef->time))) {
    ...
    // orphan block: 所有顶上没有其他被引用的block。
 ...
 * if it is greater than the current one, then the mining is performed to generate the most optimal hash
 创建一个block
 填充各个field：type、in、out、sign、key
 如果是mining，生成随机数填充
 添加到存储系统
 发送到网络：dnet和pool
 
 fields：input fields, output fields，按先后排放。
 inputsCount: input field个数
 outputsCount: output field个数
 fee：手续费
 send_time：空的时候为普通块，非空为交易块（本时间片末尾）。
 */
                                     ...
                                     xdag_amount_t fee, xtime_t send_time, xdag_hash_t block_hash_result)
{
    ...
    for (i = 0; i < inputsCount; ++i) {
        ...
        // 搜索keysnum数组，看ref->n_our_key是不是已经记录过了
        for (j = 0; j < nkeysnum && ref->n_our_key != keysnum[j]; ++j)
            ;

        if (j == nkeysnum) {
            if (outsigkeyind < 0 && ref->n_our_key == defkeynum) {
                outsigkeyind = nkeysnum; // default key在keysnum数组中的下标
            }
            keysnum[nkeysnum++] = ref->n_our_key; // keysnum: input fields中用到的自己的不同的key的编号
        }
    }
    pthread_mutex_unlock(&g_create_block_mutex);
    ...
    /*
     keys：自己本地已知的秘钥列表
     nkeys：keys列表长度
     keysnum：所有用到自己的key的编号列表
     nkeysnum：keysum数组长度
     outsigkeyind：default key在keysnum中index，out sign key index，用来对签出signout字段。
     
     3 * nkeysnum: 每个用到的key, 都要占用三个field，1个存key本身，2个存sign结果
     (outsigkeyind < 0 ? 2 : 0):
        outsigkeyind=-1表示没有在input中找到使用defkey的field，这时需要再增加两个field保存用defkey做的signout签名结果
        如果在input中已经有了使用defkey的field，则默认的3 * nkeysnum已经包含了这个计算在内，不需要额外增加了
     */
    ...
    for (;;) {
        ...
        if (g_xdag_state == XDAG_STATE_REST) {
            ...
            if (t > (g_xdag_last_received << 10) && t - (g_xdag_last_received << 10) > 3 * MAIN_CHAIN_PERIOD) {
                ...
                if (!g_light_mode && t - conn_time >= 2 * MAIN_CHAIN_PERIOD && !memcmp(&g_xdag_stats.difficulty, &g_xdag_stats.max_difficulty, sizeof(xdag_diff_t))) {
                    sync_time = t;
                }
                ...
                if (t - (g_xdag_xfer_last << 10) <= 2 * MAIN_CHAIN_PERIOD + 4) {
                    ...
                    g_xdag_state = (g_xdag_mining_threads > 0 ? (g_xdag_testnet ? XDAG_STATE_MTST : XDAG_STATE_MINE)
...
static int32_t find_and_verify_signature_out(struct xdag_block *bref, struct xdag_public_key *public_keys, const int keysCount)
{
    ...
    for (int k = 0; j < XDAG_BLOCK_FIELDS; ++j) {
        if (xdag_type(bref, j) == XDAG_FIELD_SIGN_OUT && (++k & 1) && valid_signature(bref, j, keysCount, public_keys) >= 0) {
            break;
        }
    }
