...
int xdag_do_xfer(void *outv, const char *amount, const char *address, const char *remark, int isGui)
{
    ...
    // 遍历our blocks，收集所有有余额的block放入infield，把目标放入outfield，有可能产生多个block
...
/*
    打印当前磁盘中所有block的概要信息：address，amount
 */
int out_balances()
{
    char address[33] = {0};
    /*
        TODO: 在out_balances_callback函数中会动态分配内存，存储概要信息，为了后面用地址排序后打印，但是内存没有释放。大概因为这是命令行一次性使用。
     */
