...
int xdag_init(int argc, char **argv, int isGui)
{
    ...
    if (xdag_transport_start(transport_flags, transport_threads, bindto, n_addrports, addrports))
        return -1;
    ...
    if (!is_miner) {
        ...
        if (xdag_netdb_init(pubaddr, n_addrports, addrports))
            return -1;
    }
    ...
    if (is_rpc) {
        ...
        if (!!xdag_rpc_service_start(rpc_port))
            return -1;
    }
    xdag_mess("Starting blocks engine...");
    if (xdag_blocks_start(g_is_pool, mining_threads_count, !!miner_address))
        return -1;

    if (!g_disable_mining) {
        xdag_mess("Starting pool engine...");
        if (xdag_initialize_mining(pool_arg, miner_address))
            return -1;
    }
