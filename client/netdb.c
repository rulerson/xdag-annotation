...
/*
 如果h存在于rbtree中，设置h的flag到tree中结点（if HOST_SET）
 如果h不存在与rbtree中
    h加入到rbtree中
    不在database文件的，再加入到文件中
 如果是白名单host，则继续加入到g_xdag_white_ips列表中
 */
...
static struct host *ipport2host(struct host *h, const char *ipport, int flags)
{
    ...
    if (sscanf(ipport, "%u.%u.%u.%u:%u", ip, ip + 1, ip + 2, ip + 3, ip + 4) != 5 || (ip[0] | ip[1] | ip[2] | ip[3]) & ~0xff || ip[4] & ~0xffff)
        return 0;
    ...
    while (fscanf(f, "%63s", str) == 1) {
        ...
        if (flags & HOST_CONNECTED && h0.flags & HOST_CONNECTED) {
            ...
            if (i == n_ips && i < MAX_BLOCKED_IPS * MAX_ALLOWED_FROM_IP) {
                ...
                ips_count[i] = 1;
                n_ips++;
            } else if (i < n_ips && ips_count[i] < MAX_ALLOWED_FROM_IP && ++ips_count[i] == MAX_ALLOWED_FROM_IP && n_blocked < MAX_BLOCKED_IPS) {
...
int xdag_netdb_init(const char *our_host_str, int npairs, const char **addr_port_pairs)
{
    ...
    if (!g_xdag_blocked_ips || !g_xdag_white_ips)
        return -1;
...
/* reads hosts data sent by another host */
