...
/*
 发送指定时间范围的所有block过去，接着发送一个统计block，把本地的sum信息也发过去。
 前面的block是普通类型的，后面这个field0-type是XDAG_FIELD_NONCE类型，field1-type是XDAG_MESSAGE_BLOCKS_REPLY类型。
 但这里并没有直接发送，只是把这些字节放到了connection的buffer里，等待后续专门的发送线程去发送。
 */
...
/*
 伪块，接受一些统计和命令信息，包格式参考：Protocol.md ## 5.Transport layer
     响应相关的命令请求
     且更新网络统计信息
 */
static int process_transport_block(struct xdag_block *received_block, void *connection)
{
    ...
    switch (xdag_type(received_block, 1)) {     // 伪块，第0个field type=0，第1个field中的type为具体命令类型。
        ...
        case XDAG_MESSAGE_BLOCKS_REPLY: {
            if (atomic_compare_exchange_strong_explicit_uint_least64(
                                                                     &reply_id, (uint64_t *)received_block->field[1].hash, reply_id_private, memory_order_relaxed,
...
int xdag_transport_start(int flags, int nthreads, const char *bindto, int npairs, const char **addr_port_pairs)
{
    ...
    if (!res) {
        ...
        if (version)
            dnet_set_self_version(version + 1);
    }
...
int xdag_generate_random_array(void *array, unsigned long size) { return dnet_generate_random_array(array, size); }

/*
 随机选择一个线程中的连接,
 发送请求出去，顺带把自己的统计信息和已知的网络结点带过去
 等待结果(最长等待64秒）
 */
 ...
 */
int xdag_request_blocks(xtime_t start_time, xtime_t end_time, void *data, void *(*callback)(void *block, void *data))
{
...
int xdag_net_command(const char *cmd, void *out) { return dnet_execute_command(cmd, out); }

/* sends the package, conn is the same as in function dnet_send_xdag_packet
 发送数据，如果没有当前连接，就随机选择一个，如果有就用当前连接
 */
