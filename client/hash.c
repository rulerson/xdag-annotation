...
/*
 这个函数做attempts次数（正常是4096）的hash计算，寻找能得到最小hash的nonce并返回。
 ctxv: hash ctx，attempts次的计算完成后，这里保存了hash计算后的state。
 nonce：初始nonce
 attempts：尝试多少次寻找
 step：为nonce增加步长
 hash：计算完成后的hash值
...
uint64_t xdag_hash_final_multi(void *ctxv, uint64_t *nonce, int attempts, int step, xdag_hash_t hash)
{
    ...
    for (i = 0; i < N; ++i) {
        mctx1.A[i] = ctx1->state[0];
        mctx2.A[i] = ctx2->state[0];
        mctx1.B[i] = ctx1->state[1];
        mctx2.B[i] = ctx2->state[1];
        mctx1.C[i] = ctx1->state[2];
        mctx2.C[i] = ctx2->state[2];
        mctx1.D[i] = ctx1->state[3];
        mctx2.D[i] = ctx2->state[3];
        mctx1.E[i] = ctx1->state[4];
        mctx2.E[i] = ctx2->state[4];
        mctx1.F[i] = ctx1->state[5];
        mctx2.F[i] = ctx2->state[5];
        mctx1.G[i] = ctx1->state[6];
        mctx2.G[i] = ctx2->state[6];
        mctx1.H[i] = ctx1->state[7];
        mctx2.H[i] = ctx2->state[7];
    }
    ...
    for (j = 0; j < attempts; j += N) {
        ...
        for (i = 0; i < N; ++i) {
            memcpy(array1 + i * 128 + 56, nonce, 8);
            *nonce += step;
        }
