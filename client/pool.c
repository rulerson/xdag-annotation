...
static int register_new_miner(connection_list_element *connection)
{
    ...
    pthread_mutex_lock(&g_connections_mutex);
    /*
     在全局miner list中搜索这个miner，如果找到了，说明已有的miner重新上来了，把miner挂到当前connection上。
     */
    ...
    /*
     如果在全局miner list中没找到这个miner，如果找到了，创建一个新的miner挂到当前connection上。
     */
    if (!exists) {
        pthread_mutex_lock(&g_connections_mutex);
...
// 最多10s输出一次余额，或者task
/*
 如果connection的task index落后当前了，输出给miner最新的task，两个field
 如果距离上次输出余额超过10s，再次输出。
 */
...
int pay_miners(xtime_t time)
{
    ...
    if (!xdag_wallet_default_key(&defkey))
        return -5;
...
void update_mean_log_diff(struct connection_pool_data *conn_data, struct xdag_pool_task *task, xdag_hash_t hash)
{
    ...
    if (conn_data->task_time < task_time) {     // 计算4h平均难度
