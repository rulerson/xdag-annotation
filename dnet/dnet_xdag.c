...
struct xsector {        // 512 byte
    union {
        uint8_t byte[SECTOR_SIZE];       // 512 byte
        uint32_t word[SECTOR_SIZE / sizeof(uint32_t)];   // 512 byte
        struct xsector *next;
        struct dnet_packet_header head;      // 8 byte
    };
};
...
/*
 ...
 真实的connection：直接向connection缓冲填充数据，等待发送。
 0：随机选择一个connection，接着发送数据。
 真实connection | 1：尾部1位或1，没看懂想干什么。
 ttl值（0~255)：设置ttl值，这个逻辑与上面一个一样，没看懂。貌似是加到g_common_queue缓冲中去了。
 */
...
static void *xthread_main(void *arg)
{
    ...
    while (poll(t->fds, t->nconnections, 1) >= 0) {
        ...
        for (int n = 0; n < nmax; ++n) {
            ...
            if (t->fds[n].revents & ~(POLLIN | POLLOUT) || (conn->packets_in <= conn->dropped_in + FIRST_NSECTORS && time(0) > conn->created + BAD_CONN_TIMEOUT_SEC)) {
        ...
        skip_in:
            if (t->fds[n].revents & POLLOUT) {
                ...
                if (conn->part && conn->part->writelen) {
                    ...
                    if (conn->out_queue_size && (conn->packets_out & 1 || conn->common_queue_pos == g_common_queue_pos)) {
                        pthread_mutex_lock(&conn->mutex);
            ...
            if (conn->out_queue_size || conn->common_queue_pos != g_common_queue_pos || (conn->crypt && time(0) >= conn->crypt->last_sent + OLD_DNET_TIMEOUT_SEC)) {
                t->fds[n].events |= POLLOUT;
            }
        }
    }
...
// dnet -d -s local-bind-ip:port another-full-node-ip:port another-full-node-ip:port
